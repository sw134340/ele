            ...
            <div class="modal-body">
                <form method="post"  class="form-horizontal" role="form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label >tname:</label><input type="text" class="form-control"  name="tname" placeholder="请输入种类名称"/>
                    </div>

                        <div class="form-group">
                            <label>tinfo:</label><input type="text" class="form-control" name="tinfo" placeholder="种类信息"/>
                        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post"  enctype="multipart/form-data" class="form-horizontal" role="form">
                    ...
                    <div class="form-group">
                        <label >tname:</label>
                        <input type="text" class="form-control" name="tname" placeholder="请输入种类名称"/>
                    </div>
...
<form id="searchForm" method="get" action="${app}/storeinforest/list">
    ...
    <input name="tid" type="text" value="" placeholder="tid"/>
...
<script>
    ...
    function updateForm(ele) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#updateModal [name="tinfo"]').val(result.dataZone.obj.tinfo);
    ...
    function deleteMuliRecord() {
        ...
        if (confirm("是否删除name为" + names + "的记录")) {
            ...
            $.ajax({
                type: "DELETE",
                url: "${app}/typeofgoodsrest/opt/" + ids,
                success: function (result) {
                    gotoPage(currentPage);
                    alertTips(result.message,"alert-success");
                },
    ...
    function gotoPage(pageNum, pageSize) {
        ...
        $.ajax({
            type: "GET",
            url: "${app}/typeofgoodsrest/list?pageNum=" + pageNum + "&pageSize=" + pageSize,
            dataType: "json",
            data: $("#searchForm").serialize(),
            success: function (result) {
    ...
    function parseDataAndShow(result) {
        ...
        $.each(lists, function (index, item) {
            ...
            var td3 = $('<td></td>').text(item.tinfo);
            var addTimeTd = $('<td></td>').text(new Date(item.addTime).Format("yyyy-MM-dd HH:mm:ss"));
            var upBtnTd = $('<td></td>').html('<a class="upBtn btn btn-info btn-sm" href="${app}/typeofgoodsrest/opt/' + item.tid + '">修改</a>');
            var delBtnTd = $('<td></td>').html('<a class="delBtn btn btn-danger btn-sm" href="${app}/typeofgoodsrest/opt/' + item.tid + '">删除</a>');
            //将单元格追加到行中
